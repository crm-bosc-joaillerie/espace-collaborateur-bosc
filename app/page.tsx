...
function Login({ onEnter }:{ onEnter:(name:string)=>void }) {
  ...
  useEffect(()=>{void (async()=>{const current=await activeName();if(current){onEnter(current);return}if(savedDevice()){try{const status=await deviceStatus();setName(status.name);setRequestCode(status.requestCode||"");setStep(status.approved?"login":"pending")}catch{setStep("request")}}})()},[onEnter]);
