...
const nav = [
  ...
  { id:"discussion", label:"Discussion", icon:MessageCircle, count:3 },
...
function Pochettes({rows,onSave}:{rows:PochettesRow[];onSave:(data:Record<string,any>)=>Promise<void>}) {
  ...
  const [form,setForm]=useState({no:"",client:"",piece:"",livraison:"",etat:"En attente"});
  const months=useMemo(()=>Array.from(new Map(rows.filter(row=>row.monthKey).map(row=>[row.monthKey,row.monthLabel])).entries()).sort(([a],[b])=>a.localeCompare(b)),[rows]);
  const weeks=useMemo(()=>Array.from(new Map(rows.filter(row=>row.weekKey&&(month==="Tous les mois"||row.monthKey===month)).map(row=>[row.weekKey,row.weekLabel])).entries()).sort(([a],[b])=>a.localeCompare(b)),[rows,month]);
  ...
  const openEditor=(row?:PochettesRow)=>{const p=row||{no:"",client:"",piece:"",livraison:"",etat:"En attente"};setEditing(row||null);setFormOpen(true);setForm({no:p.no,client:p.client,piece:p.piece,livraison:p.livraison,etat:p.etat})};
  const save=async()=>{if(!form.no.trim()||!form.client.trim()||saving)return;setSaving(true);try{await onSave({...form,kind:"entry",_crmId:editing?._crmId,record_key:editing?.record_key,interventionId:form.no});setEditing(null);setFormOpen(false)}finally{setSaving(false)}};
  ...
  return <>
    ...
    {monthOnly&&<p className="mb-4 text-sm text-[#57617E]">Chaque semaine est identifiée par une couleur différente.</p>}{!hasFilter?<div className="rounded-2xl border border-dashed border-[#D9DCE5] bg-white p-10 text-center text-[#57617E]">Sélectionnez un mois ou un numéro de semaine pour afficher les pochettes.</div>:<div className="grid gap-4">{filtered.map(p=>{const photo=findPhotoSource(p.data);return <Card key={p._crmId||p.no} className={`rounded-2xl border-[#D9DCE5] shadow-none ${monthOnly ? (weekColors.get(p.weekKey) || "bg-white") : "bg-white"}`}><CardContent className="p-4 sm:p-5"><div className={`grid min-w-0 gap-4 xl:items-center ${monthOnly?"xl:grid-cols-[64px_minmax(0,1.15fr)_minmax(0,1fr)_minmax(0,.8fr)_minmax(0,.8fr)_minmax(0,.8fr)_auto_auto]":"xl:grid-cols-[64px_minmax(0,1.15fr)_minmax(0,1fr)_minmax(0,.8fr)_minmax(0,.8fr)_auto_auto]"}`}>{photo?<button type="button" onClick={()=>setPhotoPreview({src:photo,no:p.no,client:p.client})} className="group size-16 cursor-zoom-in rounded-xl text-left" aria-label={`Agrandir la photo de la pochette ${p.no}`}><img src={photo} alt={`Photo de la pochette ${p.no}`} className="size-16 rounded-xl border border-[#D9DCE5] bg-[#F7F3EC] object-cover transition group-hover:scale-105 group-focus-visible:ring-2 group-focus-visible:ring-[#57617E]"/></button>:<div className="grid size-16 place-items-center rounded-xl border border-dashed border-[#D9DCE5] text-xs text-[#57617E]">Sans photo</div>}<button type="button" className="min-w-0 text-left" onClick={()=>setSelected(p)}><p className="text-xs uppercase tracking-[.1em] text-[#57617E]">N° DE POCHETTE</p><p className="mt-1 text-lg font-semibold text-[#20253A]">{p.no}</p><p className="break-words font-medium xl:truncate">{p.client}</p></button><div className="grid min-w-0 grid-cols-2 gap-x-4 gap-y-3 sm:grid-cols-3 xl:contents"><div className="min-w-0"><p className="text-xs uppercase tracking-[.1em] text-[#57617E]">Travail</p><p className="mt-1 break-words text-sm xl:truncate">{p.piece}</p></div><div className="min-w-0"><p className="text-xs uppercase tracking-[.1em] text-[#57617E]">Livraison</p><p className="mt-1 break-words text-sm font-medium">{p.livraison||"—"}</p></div><div className="min-w-0"><p className="text-xs uppercase tracking-[.1em] text-[#57617E]">Durée</p><p className="mt-1 break-words text-sm font-medium">{p.duree||"—"}</p></div>{monthOnly&&<div className="min-w-0"><p className="text-xs uppercase tracking-[.1em] text-[#57617E]">Semaine</p>{p.weekLabel?<span className={`mt-1 inline-flex rounded-full border px-2.5 py-1 text-xs font-semibold ${weekColorClass(p.weekKey)}`}>{p.weekLabel}</span>:<p className="mt-1 text-sm">—</p>}</div>}</div><Badge variant="outline" className={`justify-self-start xl:justify-self-auto ${tone(p.etat)}`}>{p.etat}</Badge><div className="flex flex-wrap gap-2 sm:col-span-2 xl:col-span-1 xl:justify-self-end"><Button size="sm" variant="outline" onClick={()=>setSelected(p)}>Ouvrir la fiche</Button><Button size="sm" variant="outline" onClick={()=>openEditor(p)}>Modifier</Button></div></div></CardContent></Card>})}</div>}{hasFilter&&filtered.length===0&&<div className="rounded-2xl border border-dashed p-8 text-center text-sm text-[#57617E]">Aucune pochette ne correspond aux filtres sélectionnés.</div>}
