...
import { activeName, deviceStatus, login, requestDevice, savedDevice, deleteRecord, readSharedCrm, recordData, saveRecord, signOut, verifyPrincipalAdminCode, type CrmRecord, type SharedCrmData } from "@/lib/crm-sync";
...
function Login({ onEnter }:{ onEnter:(name:string)=>void }) {
  const [name,setName]=useState(""); const [pin,setPin]=useState(""); const [label,setLabel]=useState("");
  const [step,setStep]=useState<"request"|"pending"|"login">("request");
  const [requestCode,setRequestCode]=useState(""); const [busy,setBusy]=useState(false); const [error,setError]=useState("");
  useEffect(()=>{localStorage.removeItem("bosc-collaborateurs-auth");void (async()=>{const current=await activeName();if(current){onEnter(current);return}if(savedDevice()){try{const status=await deviceStatus();setName(status.name);setRequestCode(status.requestCode||"");setStep(status.approved?"login":"pending")}catch{setStep("request")}}})()},[onEnter]);
  const ask=async(e:React.FormEvent)=>{e.preventDefault();setBusy(true);setError("");try{const result=await requestDevice(name,pin,label);setName(result.name);setRequestCode(result.requestCode);setPin("");setStep("pending")}catch(err){setError(err instanceof Error?err.message:"Demande impossible.")}finally{setBusy(false)}};
  const check=async()=>{setBusy(true);setError("");try{const result=await deviceStatus();if(result.approved)setStep("login");else setError("L'administrateur n'a pas encore approuvé cet appareil.")}catch(err){setError(err instanceof Error?err.message:"Vérification impossible.")}finally{setBusy(false)}};
  const enter=async(e:React.FormEvent)=>{e.preventDefault();setBusy(true);setError("");try{onEnter(await login(pin))}catch(err){setError(err instanceof Error?err.message:"Connexion impossible.")}finally{setBusy(false)}};
  return <main className="min-h-screen bg-[#19213D] grid lg:grid-cols-[1.15fr_.85fr]">
    <section className="relative hidden lg:flex flex-col justify-between p-14 overflow-hidden"><div className="absolute inset-0 opacity-20 bg-[radial-gradient(circle_at_35%_20%,#C3C7D6,transparent_36%),radial-gradient(circle_at_80%_85%,#57617E,transparent_30%)]"/><Brand light/><div className="relative max-w-xl"><div className="text-[#C3C7D6] text-sm uppercase tracking-[.24em] mb-5">Espace équipe</div><h1 className="font-serif text-white text-5xl leading-[1.08]">L’atelier, organisé autour de chacun.</h1><p className="text-white/55 text-lg mt-6 max-w-lg">Chaque appareil doit être approuvé depuis le CRM principal.</p></div><p className="relative text-white/30 text-sm">Maison Bosc Joaillerie · Salles</p></section>
    <section className="bg-[#F7F3EC] flex items-center justify-center p-6 sm:p-12"><Card className="w-full max-w-md border-[#D9DCE5] shadow-[0_30px_80px_rgba(0,0,0,.14)] rounded-3xl"><CardHeader className="p-8 pb-4"><div className="lg:hidden mb-8"><Brand/></div><Badge variant="outline" className="w-fit border-[#D9DCE5] bg-[#F7F3EC] text-[#303851]">Connexion sécurisée</Badge><CardTitle className="font-serif text-3xl mt-4">Bonjour</CardTitle><p className="text-sm text-[#57617E] leading-relaxed">{step==="request"?"Demandez l’accès à cet appareil.":step==="pending"?"Présentez ce code à l’administrateur pour validation.":`Bienvenue ${name}. Saisissez votre code personnel.`}</p></CardHeader><CardContent className="p-8 pt-3 space-y-5">
      {step==="request"&&<form onSubmit={ask} className="space-y-5"><label className="grid gap-2 text-sm font-medium">Votre prénom<Input required value={name} onChange={e=>setName(e.target.value)} placeholder="Laurent ou Camille" className="h-12 bg-[#F7F3EC]"/></label><label className="grid gap-2 text-sm font-medium">Nom de cet appareil<Input required value={label} onChange={e=>setLabel(e.target.value)} placeholder="iPhone de Camille" className="h-12 bg-[#F7F3EC]"/></label><label className="grid gap-2 text-sm font-medium">Code personnel à 4 chiffres<Input required value={pin} onChange={e=>setPin(e.target.value.replace(/\D/g,"").slice(0,4))} inputMode="numeric" pattern="[0-9]{4}" maxLength={4} type="password" className="h-12 bg-[#F7F3EC]"/></label><Button type="submit" disabled={busy||pin.length!==4} className="h-12 w-full bg-[#19213D] hover:bg-[#303851]">{busy?"Envoi…":"Demander l’accès"} <ChevronRight/></Button></form>}
      {step==="pending"&&<div className="space-y-5"><div className="rounded-xl border bg-white p-5 text-center"><p className="text-sm">Demande pour {name}</p><p className="mt-2 font-mono text-3xl tracking-widest">{requestCode||"En attente"}</p></div><Button onClick={()=>void check()} disabled={busy} className="h-12 w-full bg-[#19213D] hover:bg-[#303851]">Vérifier l’approbation</Button></div>}
      {step==="login"&&<form onSubmit={enter} className="space-y-5"><label className="grid gap-2 text-sm font-medium">Code personnel<Input required value={pin} onChange={e=>setPin(e.target.value.replace(/\D/g,"").slice(0,4))} inputMode="numeric" pattern="[0-9]{4}" maxLength={4} type="password" className="h-12 bg-[#F7F3EC] text-center text-lg tracking-[.45em]"/></label><Button type="submit" disabled={busy||pin.length!==4} className="h-12 w-full bg-[#19213D] hover:bg-[#303851]">Ouvrir mon espace <ChevronRight/></Button></form>}
      {error&&<p role="alert" className="rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-700">{error}</p>}
      <InstallAppButton/><a href={MAIN_CRM_URL} target="_blank" rel="noreferrer" className="flex h-11 w-full items-center justify-center gap-2 rounded-xl border border-[#D9DCE5] bg-white text-sm font-medium text-[#303851]">Ouvrir le CRM principal <ExternalLink className="size-4"/></a>
...
export default function Home() {
  ...
  const navigate=(module:View)=>{
    if(module==="admin"){setView("admin");setMobile(false);return true;}
