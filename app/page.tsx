...
import { deleteRecord, readSharedCrm, recordData, saveRecord, sendOtp, signOut, verifyOtp, verifyPrincipalAdminCode, getStaffProfile, type CrmRecord, type SharedCrmData } from "@/lib/crm-sync";
...
function Login({ onEnter }:{ onEnter:(name:string,isAdmin:boolean)=>void }) {
  ...
  useEffect(()=>{
    let active=true;
    void getStaffProfile().then(profile=>{
      if(!active)return;
      if(profile)onEnter(profile.full_name||"Collaborateur",profile.role==="owner"||profile.role==="admin");
  ...
  const requestOtp=async(e:React.FormEvent)=>{
    ...
    catch{setError("Envoi impossible. Vérifiez votre adresse ou demandez l'activation de votre compte.")}
    finally{setBusy(false)}
  };
  const validateOtp=async(e:React.FormEvent)=>{
    e.preventDefault();if(otp.length!==8||busy)return;
    setBusy(true);setError("");
    try{
      await verifyOtp(email,otp);
      const profile=await getStaffProfile();
      if(!profile){await signOut();setError("Ce compte n'est pas encore autorisé dans le CRM collaborateurs.");return}
      onEnter(profile.full_name||"Collaborateur",profile.role==="owner"||profile.role==="admin");
    }catch{setError("Code invalide, expiré ou accès non autorisé.")}
    finally{setBusy(false)}
  };
  return <main className="min-h-screen bg-[#19213D] flex items-center justify-center p-6">
    <Card className="w-full max-w-md rounded-3xl bg-[#F7F3EC]"><CardHeader><Brand/><CardTitle className="font-serif text-3xl mt-4">CRM Collaborateurs</CardTitle><p className="text-sm text-[#57617E]">Connectez-vous avec votre compte nominatif autorisé.</p></CardHeader>
      <CardContent className="space-y-5">
        {checking?<p role="status">Vérification de la session…</p>:step==="email"?
          <form onSubmit={requestOtp} className="space-y-4"><label className="grid gap-2">Adresse e-mail professionnelle<Input type="email" required autoComplete="email" value={email} onChange={e=>setEmail(e.target.value)}/></label><Button type="submit" disabled={busy} className="w-full bg-[#19213D] text-white">Recevoir mon code</Button></form>:
          <form onSubmit={validateOtp} className="space-y-4"><label className="grid gap-2">Code reçu par e-mail<Input value={otp} onChange={e=>setOtp(e.target.value.replace(/\\D/g,"").slice(0,8))} inputMode="numeric" pattern="[0-9]{8}" maxLength={8} autoComplete="one-time-code"/></label><Button type="submit" disabled={busy||otp.length!==8} className="w-full bg-[#19213D] text-white">Valider le code</Button><Button type="button" variant="outline" onClick={()=>setStep("email")}>Changer d’adresse</Button></form>}
...
export default function Home() {
  ...
  const [name,setName]=useState(""); const [isAdmin,setIsAdmin]=useState(false); const [view,setView]=useState<View>("accueil"); const [mobile,setMobile]=useState(false);
